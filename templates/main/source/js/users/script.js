			...
			input.addEventListener('change', function (e) {
				if (input.files && input.files[0]) {
					...
					for (let i = 0; i < input.files.length; i++) {
						...
						var fileResultItem = document.createElement("div");
						fileResultItem.classList.add("file__result-item");
						var iconPdf = document.createElement("span");
						iconPdf.classList.add("icon", "icon_pdf");
						var fileResultText = document.createElement("span");
						fileResultText.classList.add("file__result-text");
						fileResultText.innerHTML = name;
						var fileResultSize = document.createElement("span");
						fileResultSize.classList.add("file__result-size");
						fileResultSize.innerHTML = size + " мб";
						var iconClose = document.createElement("span");
						iconClose.classList.add("icon", "icon_close");
						fileResultItem.appendChild(iconPdf);
						fileResultItem.appendChild(fileResultText);
						fileResultItem.appendChild(fileResultSize);
						fileResultItem.appendChild(iconClose);
						result.appendChild(fileResultItem);

						iconClose.addEventListener("click", function () {
							fileResultItem.parentNode.removeChild(fileResultItem);
							// $(fileResultItem).remove();
						})
					}
